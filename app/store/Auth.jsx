      ...
      (set, get) => ({
        ...
        toggleAdmin: async (userId, makeAdmin) => {
          try {
            ...
            return { success: false, message: "Failed to toggle admin privileges" };
          }
        },
        ...
        getAllUsers: async (queryParams = {}) => {
          try {
            const { accessToken } = get();
            const { page = 1, limit = 20, search, isActive } = queryParams;
            ...
            const response = await fetch(`${SERVER_API}/auth/admin/users?${searchParams.toString()}`, {
              method: "GET",
              headers: {
                Authorization: `Bearer ${accessToken}`,
              },
